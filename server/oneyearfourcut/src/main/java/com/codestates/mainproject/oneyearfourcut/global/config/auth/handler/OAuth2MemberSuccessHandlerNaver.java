...
public class OAuth2MemberSuccessHandlerNaver extends SimpleUrlAuthenticationSuccessHandler {
...
/*        // kakao는 kakao_account에 유저정보가 있다. (email)
        ...
        Long naverId = (Long) naverResponse.get("id");
        String email = (String) naverResponse.get("email");
        String nickname = (String) naverResponse.get("nickname");
        String profile = (String) naverResponse.get("profile_image_url");


        List<String> authorities = List.of("USER"); //일단 모든유저 역할 통일

        saveMember(email, nickname, profile, naverId);
        redirect(request, response, email, authorities);
    }

    private void saveMember(String email, String nickname, String profile, Long naverId) {
